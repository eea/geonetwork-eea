...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gml="http://www.opengis.net/gml"
  ...
  <!-- INSPIRE / TG2 / Require nilReason attribute to be innaplicable or unknown when version is empty. -->
  ...
  <xsl:template match="gmd:*[@codeListValue]">
    <xsl:copy>
      ...
      <xsl:attribute name="codeList">
        <xsl:value-of
          select="concat('https://standards.iso.org/ittf/PubliclyAvailableStandards/ISO_19139_Schemas/resources/codelist/ML_gmxCodelists.xml#',local-name(.))"/>
      </xsl:attribute>
    </xsl:copy>
  </xsl:template>
